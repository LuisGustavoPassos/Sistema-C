    ...
    public partial class CadastrosForm : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            using (SqlConnection cn = new SqlConnection(Conn.StrCon))
            {
                ...
                var sqlQuery = $"SELECT * FROM fornes WHERE {palavraFiltro} = '{palavraPesquisa}'"; 
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection cn = new SqlConnection(Conn.StrCon))
                {
                    ...
                    var sqlQuery = "SELECT * FROM fornes";
                    using (SqlDataAdapter da = new SqlDataAdapter(sqlQuery, cn))
                    {
                        using (DataTable dt = new DataTable())
                        {
            ...
            else
            {
                ...
                try
                {
                    using (SqlConnection cn = new SqlConnection(Conn.StrCon))
                    {
                        ...
                        string sqlQuery = $"exec adicionarFornecedor '{registro}', '{nome}', '{uf}', '{cidade}', '{telefone}', '{email}', '{registroEstadual}'";
